...
datetime_now = datetime.now(cet)
datetime_now_str = datetime_now.strftime(time_format)
...
def insert_defaults():
    ...
    for perm in data["permissions"]:
        if not Permission.get_by_name(perm["name"]):
    ...
    for role in data["roles"]:
        if not Role.get_by_name(role["name"]):
            new_role = Role(name=role["name"], description=role["description"], color=role["color"])
            ...
            for perm_name in role.get("permissions", []):
                ...
                if permission:
                    role_permission = RolePermission(roleId=new_role.roleId, permissionId=permission.permissionId)
                    db.session.add(role_permission)
            db.session.commit()
    
    # Insert game types
    for game_type in data["gameTypes"]:
        if not GameType.get_by_name(game_type["name"]):
            new_game_type = GameType(name=game_type["name"], description=game_type["description"], color=game_type["color"])
    ...
    for game in data["games"]:
        if not Game.get_by_name(game["name"]):
            game_game_type = GameType.get_by_name(game["gameType"])
            if game_game_type:
                new_game = Game(name=game["name"], gameTypeId=game_game_type.gameTypeId, color=game["color"])
                db.session.add(new_game)
    
    db.session.commit()

    # Insert listings and their prices
    for listing in data["listings"]:
        game = Game.get_by_name(listing["game"])
        if game:
            # Add price first
            price = listing["price"]
            ...
            new_listing = Listing(
                ...
                isActive=listing.get("isActive", True),
                startDate=datetime.fromisoformat(listing["startDate"]),
                endDate=datetime.fromisoformat(listing["endDate"]) if listing.get("endDate") else None
            )
            db.session.add(new_listing)
            db.session.commit()
            
            # Add role access to listings
            for role_name in listing["roles"]:
                role = Role.get_by_name(role_name)
                if role:
                    role_listing = RoleListing(roleId=role.roleId, listingId=new_listing.listingId)
                    db.session.add(role_listing)
    
    db.session.commit()

    # Insert discounts and link them to intended listings
    for discount in data["discounts"]:
        # Create the discount
        new_discount = Discount(
            name=discount["name"],
            discount=discount["discount"],
            startDate=datetime.fromisoformat(discount["startDate"]),
            endDate=datetime.fromisoformat(discount["endDate"]) if discount.get("endDate") else None,
            isActive=discount.get("isActive", True)
        )
        db.session.add(new_discount)
        db.session.commit()
        
        # Link the discount to intended listings using DiscountIntent
        for listing_name in discount.get("intendedListings", []):
            listing = Listing.get_by_name(listing_name)
            if listing:
    ...
    for user in data["users"]:
        if not User.get_by_username(user["username"]):
            new_user = User(
            ...
            for user_role in user["roles"]:
                role = Role.get_by_name(user_role)
                if role:
                    user_role_entry = UserRole(new_user.userId, role.roleId)
...
with app.app_context():
    #db.drop_all()
    db.create_all()
    #insert_defaults()  # Keep commented out to avoid inserting default data on every start
